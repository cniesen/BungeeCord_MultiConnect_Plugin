...
public class ConfigurationUtilsTest extends TestBase {
    ...
    public void testHashInetSocketAddress() throws UnknownHostException, NoSuchAlgorithmException {
        Assert.assertEquals("zVRYf6AvCi94REMB", ConfigurationUtils.hashInetSocketAddress(mockPlugin(), getInetSocketAddress(), new HashSet()));
        Assert.assertEquals("RYf6AvCi94REMB53", ConfigurationUtils.hashInetSocketAddress(mockPlugin(), getInetSocketAddress(), new HashSet(Arrays.asList("zVRYf6AvCi94REMB", "VRYf6AvCi94REMB5"))));
    }
}